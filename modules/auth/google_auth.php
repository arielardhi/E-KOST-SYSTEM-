...
if (isset($_GET['code'])) {
    ...
    if ($profile_http_code === 200 && !empty($email)) {
        try {
            ...
            if ($user) {
                ...
                // Simpan user baru (status langsung 'verified' untuk Google signup)
                $stmt_insert = $pdo->prepare("INSERT INTO users (username, password, email, role, full_name, avatar, phone, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'verified')");
                $stmt_insert->execute([
                    $username,
                    $hashed_password,
                    $email,
