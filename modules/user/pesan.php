...
// Get contact list (dengan last message & unread count)
$stmt = $pdo->prepare("
...
// Jika ada receiver baru yang belum di kontak list, tambahkan
$receiver_info = null;
if ($receiver_id) {
...
.typing-dots span { display: inline-block; width: 7px; height: 7px; background: #999; border-radius: 50%; margin: 0 2px; animation: blink 1.2s infinite; }
        ...
        <div class="col-md-8 col-lg-9 d-flex flex-column h-100">
            <?php if ($receiver_id && $receiver_info): ?>
                ...
                <div class="p-3 border-bottom d-flex align-items-center gap-3" style="background:#fff;">
                    ...
                    <?php if (!empty($receiver_info['phone'])): ?>
                        ...
                        $wa_msg  = urlencode("Halo, saya tertarik dengan kost Anda. Apakah masih tersedia?");
                        ?>
                        <a href="https://wa.me/<?= $phone ?>?text=<?= $wa_msg ?>" target="_blank" class="wa-link">
            ...
            <?php else: ?>
                <div class="flex-grow-1 d-flex flex-column align-items-center justify-content-center text-muted">
                    ...
                    <p class="mt-3">Pilih kontak untuk mulai chatting</p>
                </div>
            <?php endif; ?>
        </div>
