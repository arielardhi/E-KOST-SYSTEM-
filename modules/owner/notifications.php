...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['mark_read'])) {
        $pdo->prepare("UPDATE notifikasi SET is_read = 1 WHERE id = ? AND user_id = ?")->execute([(int)$_POST['mark_read'], $user_id]);
    } elseif (isset($_POST['mark_all_read'])) {
        $pdo->prepare("UPDATE notifikasi SET is_read = 1 WHERE user_id = ?")->execute([$user_id]);
    } elseif (isset($_POST['delete_notif'])) {
        $pdo->prepare("DELETE FROM notifikasi WHERE id = ? AND user_id = ?")->execute([(int)$_POST['delete_notif'], $user_id]);
    }
...
$cnt_stmt->execute([$user_id]); $unread_total = (int)$cnt_stmt->fetchColumn();
$tot_stmt = $pdo->prepare("SELECT COUNT(*) FROM notifikasi WHERE user_id = ?");
$tot_stmt->execute([$user_id]); $total_notif = (int)$tot_stmt->fetchColumn();

function owner_notif_icon($message) {
    $msg = strtolower($message);
    if (strpos($msg,'booking')!==false||strpos($msg,'pesanan')!==false) return ['bi-calendar-plus-fill','#00B4BA'];
    if (strpos($msg,'bayar')!==false||strpos($msg,'pembayaran')!==false) return ['bi-cash-stack','#059669'];
    if (strpos($msg,'ditolak')!==false||strpos($msg,'cancel')!==false) return ['bi-x-circle-fill','#DC2626'];
    if (strpos($msg,'pesan')!==false||strpos($msg,'chat')!==false) return ['bi-chat-dots-fill','#D97706'];
    if (strpos($msg,'review')!==false||strpos($msg,'rating')!==false) return ['bi-star-fill','#D97706'];
...
<div class="container py-4">
    <div class="d-flex align-items-start justify-content-between mb-4 flex-wrap gap-3">
        <div>
            <h2 class="fw-bold mb-1" style="color:var(--dark)">
                ...
                <?php if($unread_total>0): ?><span class="badge ms-1" style="background:var(--danger);font-size:.7rem;vertical-align:middle"><?=$unread_total?></span><?php endif; ?>
            </h2>
            <p class="text-muted mb-0">Aktivitas terbaru kost Anda</p>
        </div>
        <form method="POST"><button type="submit" name="mark_all_read" class="btn btn-outline-secondary btn-sm"><i class="bi bi-check2-all me-1"></i>Tandai Semua Dibaca</button></form>
    </div>
    ...
    <div class="d-flex gap-2 mb-4 flex-wrap">
        ...
        <a href="?filter=unread" class="notif-filter-btn <?=$filter==='unread'?'active':''?>">Belum Dibaca<?php if($unread_total>0): ?> <span class="badge bg-danger ms-1" style="font-size:.65rem"><?=$unread_total?></span><?php endif; ?></a>
    ...
    <div class="card text-center p-5">
        ...
        <p class="text-muted">Aktivitas kost Anda akan muncul di sini</p>
    </div>
    <?php else: ?>
        ...
        <div class="card notif-item <?=!$notif['is_read']?'unread':''?>">
            <div class="card-body py-3">
                <div class="d-flex align-items-start gap-3">
                    <div class="notif-icon-wrap" style="background:<?=$color?>18;"><i class="bi <?=$icon?>" style="color:<?=$color?>"></i></div>
                    <div class="flex-grow-1 min-w-0">
                        <div class="d-flex justify-content-between align-items-start gap-2">
                            <p class="mb-1 fw-<?=!$notif['is_read']?'700':'500'?>" style="color:var(--text-main);line-height:1.5;"><?=htmlspecialchars($notif['message'])?></p>
                            <?php if(!$notif['is_read']): ?><div class="unread-dot mt-1"></div><?php endif; ?>
                        </div>
